...
* `att_source()` and `att_update()` now accept a character vector for `author` (e.g., `c("Smith, Jane", "Doe, John")`). Multiple authors are joined with `" and "` in BibTeX output, following standard BibTeX convention. Single-string authors are unchanged.
